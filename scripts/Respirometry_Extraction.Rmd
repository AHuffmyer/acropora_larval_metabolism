...
file.names<-basename(list.files(path = path.p, pattern = "csv$", recursive = TRUE)) #list all xlsx file names in the folder and subfolders
```
...
colnames(Resp.R) <- c("Date", "Run","Sample.ID","Chamber.ID","Intercept", "perc.air.L.min")

Resp.Rb <- data.frame(matrix(NA, ncol=6))
colnames(Resp.Rb) <- c("Date", "Run","Sample.ID","Chamber.ID","Intercept", "perc.air.L.min")

#photosynthesis 
Photo.R <- data.frame(matrix(NA, ncol=6))
colnames(Photo.R) <- c("Date", "Run","Sample.ID","Chamber.ID","Intercept", "perc.air.L.min")

Photo.Rb <- data.frame(matrix(NA, ncol=6))
colnames(Photo.Rb) <- c("Date", "Run", "Sample.ID","Chamber.ID","Intercept", "perc.air.L.min")
```
...
colnames(oxygen)[colnames(oxygen) == 'perc.air.L.min'] <- 'R.perc.air.L.min' #rename to specify R
...
colnames(oxygen)[colnames(oxygen) == 'perc.air.L.min'] <- 'P.perc.air.L.min' #rename to specify P
```
...
plot(blank_data$Run, blank_data$R.umol.min, xlab="Run", ylab="umol O2 min-1") #blanks during dark phase
plot(blank_data$Run, blank_data$P.umol.min, xlab="Run", ylab="umol O2 min-1") #blanks during light phase
