...
Issues as of 28 Nov: 
- Run 9 and 10 short time frame, check run times 
- Run 7 D6 slope at end not representative 
- Run 8 B4 slope at end not representative 
- Run 8 B5 slope at end not representative 


Filter out 9, 10 11, and 12? 

Try running respo rates across entire frame of measurements (light + dark) to see how that compares 
...
Look into blanks changing in later runs - maybe use average from previous runs for runs 12-16? 
```{r}
#list<-c(1:8)
#blank_mean<-oxygen%>%
  #filter(Run %in% list)%>%
  #summarise(mean.R.blank=mean(R.Blank.umol.min), mean.P.blank=mean(P.Blank.umol.min))

#add blank values for runs 12-16 as mean value, leave all others  
#oxygen<-oxygen%>%
  #mutate(R.Blank.umol.min=ifelse(Run>9, blank_mean$mean.R.blank, R.Blank.umol.min))%>%
  #mutate(P.Blank.umol.min=ifelse(Run>9, blank_mean$mean.P.blank, P.Blank.umol.min))
